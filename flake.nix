  ...
  outputs = { self, nixpkgs, rust-overlay, flake-utils, ... }:
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        ourNodePackages = pkgs.nodePackages_latest;
      in
      {
        devShells.default = pkgs.mkShell rec {
          nativeBuildInputs = with pkgs; [
            ...
            # mold-wrapped # doesn't work with wasm
            clang_16 # maybe for lld, which should be used for wasm by default?

            # the cloudflare cli & deps
            ourNodePackages.wrangler
            ourNodePackages.nodejs
            worker-build
            wasm-pack
